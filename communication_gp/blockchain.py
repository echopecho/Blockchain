...
def mine():
    ...
    if blockchain.valid_proof(block_string, proof):
        blockchain.new_transaction(sender="0", recipient=node_identifier, amout=1)
        ...
        response = {"message": "Provided proof is invalid"}
...
def full_chain():
    response = {"valid-chain": blockchain.valid_chain(blockchain.chain)}
    return jsonify(response), 200
...
def new_block():
    ...
    if received_block["index"] == old_block["index"] + 1:
        ...
        for neighbor in blockchain.nodes:
            ...
            if blockchain.valid_chain(neighbor_chain):
